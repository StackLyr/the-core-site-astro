...
export async function allowPublicRequest(request, env, scope, maximum, windowMs) {
  if (!env.DB || !env.ADMIN_SESSION_SECRET) return false;
  const ipHash = await adminIpHash(request, env);
  const now = new Date();
  const since = new Date(now.getTime() - windowMs).toISOString();
  const result = await env.DB.prepare(`INSERT INTO public_request_limits (id, scope, ip_hash, created_at)
    SELECT ?, ?, ?, ? WHERE (SELECT COUNT(*) FROM public_request_limits WHERE scope = ? AND ip_hash = ? AND created_at > ?) < ?`)
    .bind(crypto.randomUUID(), scope, ipHash, now.toISOString(), scope, ipHash, since, maximum).run();
  if (Math.random() < 0.01) {
    await env.DB.prepare('DELETE FROM public_request_limits WHERE created_at < ?').bind(new Date(now.getTime() - 48 * 60 * 60 * 1000).toISOString()).run();
  }
